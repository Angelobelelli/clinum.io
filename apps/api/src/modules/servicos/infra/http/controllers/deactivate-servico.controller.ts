...
@Controller('servicos')
export class DeactivateServicoController {
  constructor(
    private readonly deactivateServicoUseCase: DeactivateServicoUseCase,
    private readonly getServicoUseCase: GetServicoUseCase,
  ) {}

  @Patch(':servicoId/desativar')
  @RequirePermission('servico', 'deactivate')
  async deactivate(@Param('servicoId') servicoId: string) {
    const result = await this.deactivateServicoUseCase.execute({ servicoId });
